...
const SAMPLE_RESTAURANTS = [
  // Italian restaurants
  { name: 'Mario\'s Italian Bistro', address: '123 Main St', lat: 37.7749, lng: -122.4194, rating: 4.5, price_level: 3, cuisine: 'italian' },
  { name: 'Bella Vista Pizzeria', address: '456 Oak Ave', lat: 37.7849, lng: -122.4094, rating: 4.2, price_level: 2, cuisine: 'italian' },
  { name: 'Trattoria Romana', address: '789 Pine St', lat: 37.7649, lng: -122.4294, rating: 4.7, price_level: 4, cuisine: 'italian' },
  
  // Mexican restaurants
  { name: 'El Mariachi Cantina', address: '321 Mission St', lat: 37.7849, lng: -122.4094, rating: 4.3, price_level: 2, cuisine: 'mexican' },
  { name: 'Casa de Tacos', address: '654 Valencia St', lat: 37.7549, lng: -122.4194, rating: 4.1, price_level: 1, cuisine: 'mexican' },
  { name: 'La Cocina Mexicana', address: '987 Castro St', lat: 37.7949, lng: -122.3994, rating: 4.6, price_level: 3, cuisine: 'mexican' },
  
  // Chinese restaurants
  { name: 'Golden Dragon Restaurant', address: '147 Grant Ave', lat: 37.7649, lng: -122.4094, rating: 4.0, price_level: 2, cuisine: 'chinese' },
  { name: 'Panda Express', address: '258 Market St', lat: 37.7849, lng: -122.4194, rating: 3.8, price_level: 1, cuisine: 'chinese' },
  { name: 'Imperial Palace', address: '369 Geary Blvd', lat: 37.7749, lng: -122.4294, rating: 4.4, price_level: 3, cuisine: 'chinese' },
  
  // American restaurants
  { name: 'The Burger Joint', address: '741 Union St', lat: 37.7549, lng: -122.4094, rating: 4.2, price_level: 2, cuisine: 'american' },
  { name: 'Steakhouse Grill', address: '852 Fillmore St', lat: 37.7849, lng: -122.4294, rating: 4.5, price_level: 4, cuisine: 'american' },
  { name: 'Diner 24/7', address: '963 Divisadero St', lat: 37.7649, lng: -122.4394, rating: 3.9, price_level: 1, cuisine: 'american' }
];
...
function generateMenu(restaurantName: string, cuisineType: string) {
  const template = MENU_TEMPLATES[cuisineType as keyof typeof MENU_TEMPLATES] || MENU_TEMPLATES.american;
  ...
  const menu = {
    categories: template.categories.map(category => ({
      name: category,
      items: template.items[category as keyof typeof template.items]?.map(item => ({
...
function generateAllergens(itemName: string) {
  const allergens = [];
...
export async function POST(request: NextRequest) {
  try {
    console.log('Starting sample restaurant population...');
    ...
    for (const restaurant of SAMPLE_RESTAURANTS) {
      try {
        const menu = generateMenu(restaurant.name, restaurant.cuisine);
    ...
    console.log(`Sample restaurant population completed. Success: ${successCount}, Errors: ${errorCount}`);
    
    return NextResponse.json({
      success: true,
      message: `Successfully populated ${successCount} sample restaurants`,
      data: { success: successCount, errors: errorCount, total: SAMPLE_RESTAURANTS.length }
    });
    
  } catch (error) {
    console.error('Error in populate-sample API:', error);
    return NextResponse.json(
      { error: 'Failed to populate sample restaurants' },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  return NextResponse.json({
    message: 'Sample restaurant population API endpoint',
    usage: 'POST to populate database with sample restaurants'
  });
}
